...
export function processRows(
	...
	for (const minuteTs of sortedMinutes) {
		...
		for (const model of models) {
			...
			const entry = {
				...
				latency: b && b.latencyCount > 0 ? b.latencySum / b.latencyCount : 0,
				cost: b ? b.cost : 0,
			};
	...
	const totals = {
		...
		successRequests: modelSummaries.reduce((s, m) => s + m.successRequests, 0),
...
// ── Empty chart data with zero lines ──────────────────────────────

export function emptyChartData(hours: number): BifrostChartData {
	const now = Date.now();
	...
	return {
		...
		series: { tokens: {}, requests: {}, latency: {}, cost: {} },
		hours,
	};
}
